              ...
              CvGBTreesParams _params, bool /*_update*/ ) //update is not supported
{
    ...
    */
    ...
    CvMat* new_responses = cvCreateMat( n, 1, CV_32F, 1);
    ...
    if (_missing_mask)
    {
        missing = cvCreateMat(_missing_mask->rows, _missing_mask->cols,
                              _missing_mask->type, 1);
        cvCopy( _missing_mask, missing);
    }

    orig_response = cvCreateMat( 1, n, CV_32F , 1);
    ...
    if (!is_regression)
    {
        ...
        class_labels = cvCreateMat(1, class_count, CV_32S, 1);
    ...
    if (_sample_idx)
    {
        ...
        switch (CV_MAT_TYPE(_sample_idx->type))
        {
            case CV_32SC1:
            {
                sample_idx = cvCreateMat( 1, sample_idx_len, CV_32S , 1);
            ...
            case CV_8U:
            {
                ...
                sample_idx = cvCreateMat( 1, active_samples_count, CV_32S , 1);
    ...
    {
        sample_idx = cvCreateMat( 1, n, CV_32S , 1);
        for (int i=0; i<n; ++i)
            sample_idx->data.i[i] = i;
    }

    sum_response = cvCreateMat(class_count, n, CV_32F, 1);
    sum_response_tmp = cvCreateMat(class_count, n, CV_32F, 1);
    cvZero(sum_response);
...
void CvGBTrees::change_values(CvDTree* tree, const int _k)
{
    ...
    for (int i=0; i<leaves_count; ++i)
    {
        ...
        CvMat* leaf_idx = cvCreateMat(1, samples_in_leaf, CV_32S, 1);
...
/*
    ...
    if (subsample_train == 0)
        subsample_train = cvCreateMat(1, n, CV_32S, 1);
    int* subsample_data = subsample_train->data.i;
    for (int i=0; i<n; ++i)
        subsample_data[i] = i;
    subsample_test = 0;
